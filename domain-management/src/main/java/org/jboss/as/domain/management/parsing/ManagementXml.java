...
import org.jboss.as.domain.management.security.DatabaseAuthenticationResourceDefinition;
import org.jboss.as.domain.management.security.DatabaseAuthorizationResourceDefinition;
import org.jboss.as.domain.management.security.DatabaseResourceDefinition;
...
public class ManagementXml {
    ...
    public void parseManagement(final XMLExtendedStreamReader reader, final ModelNode address, final Namespace expectedNs, final List<ModelNode> list,
                                   boolean allowInterfaces, boolean requireNativeInterface, boolean allowDataSourceConnection) throws XMLStreamException {
        ...
        while (reader.hasNext() && reader.nextTag() != END_ELEMENT) {
            ...
            switch (element) {
                case SECURITY_REALMS: {
                    ...
                    parseSecurityRealms(reader, managementAddress, expectedNs, list, allowDataSourceConnection);
                    break;
                }
    ...
    private void parseDataSource(final XMLExtendedStreamReader reader, final List<ModelNode> list, final ModelNode address, Namespace expectedNs) throws XMLStreamException {
        ...
        list.add(add);
        Set<Attribute> required = EnumSet.of(Attribute.NAME);
        final int count = reader.getAttributeCount();
        for (int i = 0; i < count; i++) {
            final String value = reader.getAttributeValue(i);
            if (!isNoNamespaceAttribute(reader, i)) {
    ...
    private void parseSecurityRealms(final XMLExtendedStreamReader reader, final ModelNode address,
                                     final Namespace expectedNs, final List<ModelNode> list, boolean allowDataSourceConnection)
            throws XMLStreamException {
        while (reader.hasNext() && reader.nextTag() != END_ELEMENT) {
            requireNamespace(reader, expectedNs);
                ...
                case SECURITY_REALM: {
                    switch (expectedNs) {
                        ...
                        case DOMAIN_1_3:
                            ...
                            break;
                        default:
                            parseSecurityRealm_1_4(reader, address, expectedNs, list, allowDataSourceConnection);
                            break;
                    }
                    break;
                }
    ...
    private void parseSecurityRealm_1_0(final XMLExtendedStreamReader reader, final ModelNode address, final Namespace expectedNs, final List<ModelNode> list)
        ...
        final ModelNode realmAddress = cloneRealmAddress(reader, address, list);
    ...
    private void parseSecurityRealm_1_4(final XMLExtendedStreamReader reader, final ModelNode address,
                                        final Namespace expectedNs, final List<ModelNode> list, final boolean allowDataSourceConnection)
            throws XMLStreamException {
        requireSingleAttribute(reader, Attribute.NAME.getLocalName());
        // After double checking the name of the only attribute we can retrieve it.
        final ModelNode realmAddress = cloneRealmAddress(reader, address, list);
        ...
        while (reader.hasNext() && reader.nextTag() != END_ELEMENT) {
            ...
            switch (element) {
                ...
                case SERVER_IDENTITIES:
                    ...
                    break;
                case AUTHENTICATION: {
                    parseAuthentication_1_4(reader, expectedNs, realmAddress, list, allowDataSourceConnection);
                    break;
                }
                case AUTHORIZATION:
                    parseAuthorization_1_4(reader, expectedNs, realmAddress, list, allowDataSourceConnection);
                    break;
                default: {
                    throw unexpectedElement(reader);
                }
    ...
    private ModelNode parseDatabase(XMLExtendedStreamReader reader, ModelNode realmAddress, List<ModelNode> list, String nodeName)
            throws XMLStreamException {
        ModelNode addr = realmAddress.clone().add(nodeName, DATABASE);
        ModelNode databaseNode = Util.getEmptyOperation(ADD, addr);
        ...
        for (int i = 0; i < count; i++) {
            ...
            switch (attribute) {
                case PLAIN_TEXT: {
                    DatabaseResourceDefinition.PLAIN_TEXT.parseAndSetParameter(value, databaseNode, reader);
                    break;
                }
                default: {
                    throw unexpectedAttribute(reader, i);
    ...
    private void parseDatabaseConnectionRef(XMLExtendedStreamReader reader, ModelNode addOperation, final Attribute requiredAttribute,
                                            SimpleAttributeDefinition attributeDefinition) throws XMLStreamException {

        Set<Attribute> required = EnumSet.of(requiredAttribute);
        final int count = reader.getAttributeCount();
        for (int i = 0; i < count; i++) {
            final String value = reader.getAttributeValue(i);
            if (!isNoNamespaceAttribute(reader, i)) {
            ...
            attributeDefinition.parseAndSetParameter(value, addOperation, reader);
        }
    ...
    private void parseSimpleSelect(XMLExtendedStreamReader reader, ModelNode realmAddress, List<ModelNode> list, Set<Attribute> required) throws XMLStreamException {
        ...
        for (int i = 0; i < count; i++) {
            ...
            if (!isNoNamespaceAttribute(reader, i)) {
                ...
                switch (attribute) {
                    case NAME: {
                        DatabaseResourceDefinition.TABLE_FIELD.parseAndSetParameter(value, realmAddress, reader);
                        break;
                    }
                    case USERNAME_FIELD: {
                        DatabaseResourceDefinition.USERNAME_FIELD.parseAndSetParameter(value, realmAddress, reader);
                        break;
                    }
                    case PASSWORD_FIELD: {
                        DatabaseAuthenticationResourceDefinition.PASSWORD_FIELD.parseAndSetParameter(value, realmAddress, reader);
                        break;
                    }
                    case ROLES_FILED: {
                        DatabaseAuthorizationResourceDefinition.ROLES_FIELD.parseAndSetParameter(value, realmAddress, reader);
                        break;
                    }
                    default: {
                        throw unexpectedAttribute(reader, i);
    ...
    private ModelNode cloneRealmAddress(XMLExtendedStreamReader reader, ModelNode address, List<ModelNode> list) {
    ...
    private void parseAuthentication_1_4(final XMLExtendedStreamReader reader, final Namespace expectedNs,
                                         ...
                                         final boolean allowDataSourceConnection) throws XMLStreamException {

        // Only one truststore can be defined.
        boolean trustStoreFound = false;
        // Only one local can be defined.
        boolean localFound = false;
        // Only one of ldap, properties, database or users can be defined.
        boolean usernamePasswordFound = false;
        // Only one database provider can be defined
        while (reader.hasNext() && reader.nextTag() != END_ELEMENT) {
            requireNamespace(reader, expectedNs);
            final Element element = Element.forName(reader.getLocalName());
            ...
            switch (element) {
                ...
                case DATABASE: {
                    ...
                    parseDatabaseAuthentication(reader, expectedNs, realmAddress, list, allowDataSourceConnection);
                    usernamePasswordFound = true;
                    break;
                }
                case PROPERTIES: {
    ...
    private void parseAuthorization_1_4(final XMLExtendedStreamReader reader, final Namespace expectedNs,
        ...
        while (reader.hasNext() && reader.nextTag() != END_ELEMENT) {
            ...
            switch (element) {
                ...
                case DATABASE: {
                    parseDatabaseAuthorization(reader, expectedNs, realmAddress, list, allowDataSourceConnection);
                    authzFound = true;
                    break;
                }
                default: {
    ...
    public void writeManagement(final XMLExtendedStreamWriter writer, final ModelNode management, boolean allowInterfaces)
        ...
        boolean hasConnection = management.hasDefined(LDAP_CONNECTION) || management.hasDefined(DATABASE_CONNECTION);
    ...
    private void writeAuthentication(XMLExtendedStreamWriter writer, ModelNode realm) throws XMLStreamException {
        ...
        if (authentication.hasDefined(JAAS)) {
            ...
            ModelNode database = authentication.get(DATABASE);
            ...
            if (DatabaseAuthenticationResourceDefinition.TABLE_FIELD.isMarshallable(database)) {
                ...
                DatabaseAuthenticationResourceDefinition.TABLE_FIELD.marshallAsAttribute(database, writer);
                DatabaseAuthenticationResourceDefinition.USERNAME_FIELD.marshallAsAttribute(database, writer);
                DatabaseAuthenticationResourceDefinition.PASSWORD_FIELD.marshallAsAttribute(database, writer);
            } else if (DatabaseAuthenticationResourceDefinition.SQL_SELECT_USERS.isMarshallable(database)) {
                DatabaseAuthenticationResourceDefinition.SQL_SELECT_USERS.marshallAsElement(database, writer);
            }
            writer.writeEndElement();
    ...
    private void writeAuthorization(XMLExtendedStreamWriter writer, ModelNode realm) throws XMLStreamException {
        ...
        if (authorization.hasDefined(PROPERTIES)) {
            ...
            ModelNode database = authorization.get(DATABASE);
            writer.writeStartElement(Element.DATABASE.getLocalName());
            ...
            if (DatabaseAuthorizationResourceDefinition.TABLE_FIELD.isMarshallable(database)) {
                ...
                DatabaseAuthorizationResourceDefinition.USERNAME_FIELD.marshallAsAttribute(database, writer);
                DatabaseAuthorizationResourceDefinition.ROLES_FIELD.marshallAsAttribute(database, writer);
            } else if (DatabaseAuthorizationResourceDefinition.SQL_SELECT_ROLES.isMarshallable(database)) {
                DatabaseAuthorizationResourceDefinition.SQL_SELECT_ROLES.marshallAsElement(database, writer);
            }
            writer.writeEndElement();
        }
    ...
    private void writeOutboundConnections(XMLExtendedStreamWriter writer, ModelNode management) throws XMLStreamException {
        ...
        if (management.hasDefined(LDAP_CONNECTION)) {
            ...
            for (Property prop : management.get(DATABASE_CONNECTION).asPropertyList()) {
                writeDatabaseOutboundConnection(writer, prop.getName(), prop.getValue());
            }
        }
        writer.writeEndElement();
    }

    private void writeDatabaseOutboundConnection(XMLExtendedStreamWriter writer, String name, ModelNode connection) throws XMLStreamException {
