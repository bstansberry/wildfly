...
public final class ScopedStatelessSessionInvocationHandler implements InvocationHandler, Serializable {
    ...
    private <T> T unwrap(Object proxy, Class<T> type) {
        // This is a copy of the ORM 7 StatelessSessionImpl unwrap, except we unwrap the proxy and use an i18n exception message
        checkOpen();
        ...
        throw JPA_LOGGER.cannotUnwrapStatelessSession(getClass().getName(), type.getName());
    }

    private void checkOpen() {
        try {
            Object delegate = getDelegate();
            Method checkOpen = delegate.getClass().getMethod("checkOpen");
            checkOpen.invoke(delegate);
