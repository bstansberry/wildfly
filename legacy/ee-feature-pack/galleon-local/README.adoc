...
The `wildfly-legacy-ee-feature-pack-galleon-local` module is used to control the source code and dependency management for items that
are relevant to the `wildfly-legacy-ee` feature pack *but not* to the `wildfly-ee` feature pack or the `wildfly-preview` feature pack.
...
[WARNING]
====
If something is relevant to some other feature pack besides `wildfly-legacy-ee`, *DO NOT* include it here.

* If it's relevant to *all three* of the `wildfly-ee`, `wildfly-legacy-ee` and the `wildfly-preview` feature packs, use the `wildfly-ee-feature-pack-galleon-shared` maven module.
...
* If it's only for the *pair* `wildfly-ee` and `wildfly-legacy-ee`, use the `wildfly-ee-feature-pack-galleon-shared-legacy` maven module.
* If it's only for the *pair* `wildfly-ee` and `wildfly-preview`, use the `wildfly-ee-feature-pack-galleon-shared-preview` maven module.
====

This module is a dependency of the `wildfly-leagacy-ee-galleon-pack` maven module. Its content and dependency entries are used by the build of that module when it creates its feature pack.
...
* Ideally, the `wildfly-legacy-ee-galleon-pack` maven module would depend on and unzip these zip artifacts and consume their content that way. But, at least at the time this document was written, it doesn't do this and instead just copies in source from this maven module.